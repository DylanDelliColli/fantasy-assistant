...
import {browserFixture,keyboard} from '../helpers/browser.mjs';
...
test('actual rehearsal CLI Enter/quit uses isolated files and selected own players through0/1/27/28/29',async t=>{
 ...
 await until(()=>out.includes('http://')||child.exitCode!==null);assert.match(out,/REHEARSAL/);
 const url=out.match(/http:\/\/127\.0\.0\.1:\d+/)[0],directory=out.match(/Private rehearsal state: (.+)/)[1];assert.notEqual(directory,join(cwd,'.local'));
